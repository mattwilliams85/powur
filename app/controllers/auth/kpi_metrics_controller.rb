module Auth
  class KpiMetricsController < AuthController
    ...
    def show
      ...
      @proposal_count = Lead.where(user_id: user_ids).proposals.count
    ...
    def proposals_show
      ...
      @orders = @user.leads.contract.where('contract_at >= ?', scale.days.ago)
      @proposals = @user.leads.proposals
        .where('converted_at >= ?', scale.days.ago)
