      ...
      switch(_tab){
        ...
        case "#admin-plans-ranks":
          ...
          EyeCueLab.UX.getTemplate("/templates/admin/plans/ranks/_ranks.handlebars.html", _displayData, $(".js-admin_dashboard_detail_container"), function(){
            ...
            for(var _active_qualification_path in _data.active_qualification_paths){
              var _qualification_group = _data.active_qualification_paths[_active_qualification_path];
              if(_qualification_group.length==0) return;
              var _padding = 50 *_qualification_group.length;
            ...
            $(".js-active_qualification_link").on("click", function(e){
              e.preventDefault();
              var _qualificationID = parseInt($(e.target).attr("href").replace("#",""));
              var _qualificationPath = $(e.target).attr("data-qualification-path");
              var _qualification = _data.active_qualification_paths[_qualificationPath].filter(function(_q){return _q.properties.id==_qualificationID})[0];
            ...
            for(var _rankID in _data.qualifications){
              ...
              for(var _qualification_path in _data.qualifications[_rankID]){
                var _qualification_group = _data.qualifications[_rankID][_qualification_path];
                var _padding = 50*_qualification_group.length;
            ...
            $(".js-qualification_link").on("click", function(e){
              e.preventDefault();
              var _rankID = $(e.target).parents("tr").attr("data-rank-id");
              var _qualificationID = parseInt($(e.target).attr("href").replace("#",""));
              var _qualificationPath = $(e.target).attr("data-qualification-path");
              var _qualification = _data.qualifications[_rankID][_qualificationPath].filter(function(_q){return _q.properties.id==_qualificationID})[0];
              var _popupData =[];
              _popupData = _qualification.actions.filter(function(action){return action.name==="update"})[0];
              _popupData.fields.forEach(function(field){
                field.display_name=field.name.replace(/\_/g," ");
            ...
            $(".js-rank_link").on("click", function(e){
              e.preventDefault();
              var _rankID = parseInt($(e.target).attr("href").replace("#",""));
              _ajax({
                _ajaxType:"get",
            ...
            $(".js-add_qualification").on("click", function(e){
              e.preventDefault();
              var _rankID = parseInt($(e.target).parents("tr").attr("data-rank-id"));
              _ajax({
                _ajaxType:"get",
                _url:"/a/ranks/"+_rankID,
                _callback:function(data, text){
