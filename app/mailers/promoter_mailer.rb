class PromoterMailer < ActionMailer::Base
  def invitation(invite)
    to = "#{full_name_email_safe(invite.full_name)} <#{invite.email}>"
  ...
  def reset_password(user)
    to = "#{full_name_email_safe(user.full_name)} <#{user.email}>"
  ...
  def new_quote(quote)
    to = "#{full_name_email_safe(quote.customer.full_name)} <#{quote.customer.email}>"
    merge_vars = {
  ...
  def notify_upline(user)
    ...
    to = "#{full_name_email_safe(sponsor.full_name)} <#{sponsor.email}>"
  ...
  def team_leader_downline_certification_purchase(user)
    ...
    to = "#{full_name_email_safe(team_leader.full_name)} <#{team_leader.email}>"
  ...
  def full_name_email_safe(full_name)
    full_name.gsub(/[^\w\s]/, '')
  end
end
