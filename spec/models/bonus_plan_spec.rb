...
describe BonusPlan, type: :model do
  ...
  describe '#start_year,#start_month' do
    it 'allows multiple null values on nullable columns that have a unique index' do
  ...
  describe '#active' do
    ...
    it 'returns true if the plan is the one with the latest start date' do
      past_plan = create(:bonus_plan, start_year: @year, start_month: @month - 4)
      current_plan = create(:bonus_plan, start_year: @year, start_month: @month - 2)
      future_plan = create(:bonus_plan, start_year: @year + 1, start_month: @month)
